    ...
    def compute(self):
        ...
        if cs[CS_BR_TYPE] != 0:
                if imm_b >=0x80000000:
                        ...
                        imm_b= imm_b-0xffffffff-1
                        
                else:
                        forward_branch=True
        imm_j           = RISCV.imm_j(self.inst)                
        if imm_j>=0x80000000:
                imm_j = imm_j - 0xffffffff-1           
        ...
        if forward_branch or backward_branch:
        ...
        if (backward_branch and Pipe.ID.pc_sel == PC_4)and(not(cs[CS_BR_TYPE]==7)) and (cs[CS_BR_TYPE] !=8):#backward branch taken prediction
                self.pc_next= Pipe.cpu.adder_if.op(self.pc,imm_b)
        if cs[CS_BR_TYPE] ==7: #jal always taken 
        	self.pc_next = Pipe.cpu.adder_if.op(self.pc, imm_j)
        	Pipe.cpu.ras.push(Pipe.cpu.adder_if.op(self.pc,4))
        if cs[CS_BR_TYPE] ==8: #jalr always not taken,ras prediction
                rd = RISCV.rd(self.inst)
                rs1 = RISCV.rs1(self.inst)
                imm_i           = RISCV.imm_i(self.inst)
                if rd ==0 and rs1==1 and imm_i==0:#jalr return case
                        self.pc_next, status = Pipe.cpu.ras.pop()
                        if not status:
                                self.pc_next = self.pcplus4
                elif rd==1: #jalr call case
                        self.pc_next = self.pcplus4
                        Pipe.cpu.ras.push(Pipe.cpu.adder_if.op(self.pc,4))
                else:
                        self.pc_next=self.pcplus4
        if Pipe.ID.pc_sel==PC_BRJMP and (Pipe.EX.forward_branch)and(not(Pipe.EX.c_br_type==7))and (Pipe.EX.c_br_type !=8): #forward branch taken(misprediction)
                imm_b= RISCV.imm_b(Pipe.EX.inst)
                if imm_b>=0x80000000:
                        imm_b=imm_b-0xffffffff-1
                self.pc_next= Pipe.cpu.adder_if.op(Pipe.EX.pc, imm_b)
        if Pipe.ID.pc_sel==PC_4 and (Pipe.EX.backward_branch)and(not(Pipe.EX.c_br_type==7))and (Pipe.EX.c_br_type !=8):#backward branch not taken(misprediction)
                self.pc_next = Pipe.cpu.adder_if.op(Pipe.EX.pc,4)
                
        if Pipe.EX.c_br_type==8 and Pipe.ID.ID_bubble:#jalr misprediction
...
class ID(Pipe):
    ...
    def compute(self):
        ...
        self.pc_sel         =   PC_BRJMP    if (((EX.reg_c_br_type == BR_NE  and (not Pipe.EX.alu_out)) or    \
                                               ...
                                               (EX.reg_c_br_type == BR_LTU and Pipe.EX.alu_out))) else  \
                                PC_JALR     if  EX.reg_c_br_type == BR_JR else                              \
                                PC_4
        self.fwd_op1        =   FWD_EX      if (EX.reg_rd == Pipe.ID.rs1) and self.c_rs1_oen and   \
        ...
        self.fwd_op2        =   FWD_EX      if (EX.reg_rd == Pipe.ID.rs2) and               \
                                ...
                                FWD_WB      if (WB.reg_rd == Pipe.ID.rs2) and               \
                                               ...
                                               self.c_op2_sel == OP2_RS2 else                 \
                                FWD_NONE
        self.fwd_rs2        =   FWD_EX      if (EX.reg_rd == Pipe.ID.rs2) and self.c_rs2_oen and   \
                                               (EX.reg_rd != 0) and EX.reg_c_rf_wen  else   \
        ...
        M1_M2_load_inst= self.c_dmem_en and (self.c_dmem_rw == M_XRD or self.c_dmem_rw == M_XWR) and EX.reg_c_dmem_en and (EX.reg_c_dmem_rw == M_XRD or EX.reg_c_dmem_rw == M_XWR)
        ...
        M1_M2_hazard      = M1_M2_load_inst
        # Check for mispredicted branch/jump
        self.rf_rs1_data     = Pipe.cpu.rf.read(self.rs1)
        self.rf_rs2_data     = Pipe.cpu.rf.read(self.rs2)
        EX_brjmp=False
        if (self.pc_sel == PC_4 and Pipe.EX.backward_branch) or (self.pc_sel == PC_BRJMP and Pipe.EX.forward_branch):
                EX_brjmp = True
        if(Pipe.EX.c_br_type==7):
                EX_brjmp = False
        if(Pipe.EX.c_br_type ==8) and (Pipe.EX.jump_reg_target != self.pc):
                EX_brjmp = True
        ...
        alu_op2 =       self.rf_rs2_data     if self.c_op2_sel == OP2_RS2      else \
        ...
        self.op1_data = self.pc         if self.c_op1_sel == OP1_PC       else \
                        ...
                        Pipe.M1.wbdata  if self.fwd_op1 == FWD_M1       else \
                        Pipe.M2.wbdata  if self.fwd_op1 == FWD_M2       else \
                        Pipe.WB.wbdata  if self.fwd_op1 == FWD_WB       else \
                        self.rf_rs1_data
        self.rs2_data = Pipe.EX.alu_out if self.fwd_rs2 == FWD_EX       else \
                        Pipe.M1.wbdata  if self.fwd_rs2 == FWD_M1       else \
                        Pipe.M2.wbdata  if self.fwd_rs2 == FWD_M2       else \
                        Pipe.WB.wbdata  if self.fwd_rs2 == FWD_WB       else \
                        self.rf_rs2_data
        self.op2_data = Pipe.EX.alu_out if self.fwd_op2 == FWD_EX       else \
                        Pipe.M1.wbdata  if self.fwd_op2 == FWD_M1       else \
                        Pipe.M2.wbdata  if self.fwd_op2 == FWD_M2       else \
...
class EX(Pipe):
    ...
    def compute(self):
        ...
        self.rf_rs1_data     = Pipe.cpu.rf.read(self.rs1)
        self.rf_rs2_data     = Pipe.cpu.rf.read(self.rs2)
        self.forward_branch=False
        self.backward_branch=False
        cs = csignals[opcode]
        imm_b           = RISCV.imm_b(self.inst)
        if cs[CS_BR_TYPE] != 0:
                if imm_b >0xf0000000 or cs[CS_BR_TYPE] == 8:
                        self.backward_branch=True
                        imm_b= imm_b-0xffffffff-1
                        
                else:
                        self.forward_branch=True
        ...
        # For jal and jalr instructions,  should be written to the rd
